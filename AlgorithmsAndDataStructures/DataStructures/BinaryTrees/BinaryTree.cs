...
#pragma warning restore HAA0601 // Value type to reference type conversion causing boxing allocation
        ...
        private static void DepthFirstInternalPostOrder(BinaryTreeNode<T> node, List<T> traversalPath)
        {
            if (node.Left != null)
            {
                DepthFirstInternalPostOrder(node.Left, traversalPath);
            }

            if (node.Right != null)
            {
                DepthFirstInternalPostOrder(node.Right, traversalPath);
            }
        ...
        private static void DepthFirstInternalPreOrder(BinaryTreeNode<T> node, ICollection<T> traversalPath)
        {
            ...
            if (node.Left != null)
            {
                DepthFirstInternalPreOrder(node.Left, traversalPath);
            }

            if (node.Right != null)
            {
                DepthFirstInternalPreOrder(node.Right, traversalPath);
            }
