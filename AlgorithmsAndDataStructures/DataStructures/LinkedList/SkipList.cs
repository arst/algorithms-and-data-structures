...
namespace AlgorithmsAndDataStructures.DataStructures.LinkedList
{
    public class SkipList<T> where T : IComparable<T>
    {
        ...
        public void Append(T value)
        {
            ...
            {
                while (current.Next != null && current.Next.Value.CompareTo(value) == - 1)
                {
            ...
            if (nextInLine != null)
            {
                current.Next.Next = nextInLine;
            }

            var levelsToAdd = 1;
            while (random.Next(2) == 1 && levelsToAdd < maxHeight)
            {
                levelsToAdd++;
            }
