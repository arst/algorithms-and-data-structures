...
public class CacheDoubleLinkedList
{
    ...
    {
        if (cacheEntry is null) return;

        if (cacheEntry.Previous != null) cacheEntry.Previous.Next = cacheEntry.Next;

        if (cacheEntry.Next != null) cacheEntry.Next.Previous = cacheEntry.Previous;

        if (cacheEntry == head) head = null;
