...
public class CountingBloomFilter
{
    ...
    {
        foreach (var hash in seeds.Select(seed => hashGenerator.GetHash(s, seed))) filter[hash % filter.Count] += 1;
    }
    ...
    public bool Contains(string s)
    {
        return seeds.Select(seed => hashGenerator.GetHash(s, seed))
            .All(hash => filter[(hash % filter.Count + filter.Count) % filter.Count] > 0);
    }
}