...
#pragma warning restore CA1822 // Mark members as static
    {
        ...
        for (var i = 0; i < 256; i++) codeByNGramMap.Add(((char)i).ToString(CultureInfo.InvariantCulture), i);
        ...
        if (!string.IsNullOrEmpty(temp)) compressed.Add(codeByNGramMap[temp]);
...
#pragma warning restore CA1822 // Mark members as static
    {
        if (compressed is null || compressed.Count == 0) return string.Empty;
        ...
        foreach (var code in compressed)
        {
            ...
            if (!isAlreadyDecoded && code == nGramByCodeMap.Count) entry = current + current[0];
