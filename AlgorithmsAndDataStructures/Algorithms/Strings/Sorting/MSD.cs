...
#pragma warning restore CA1822 // Mark members as static
        ...
        private static void SortInternal(IList<string> input, IList<string> auxiliary, int start, int end, int currentCharacter)
        {
            if (start >= end)
            {
                return;
            ...
            for (var i = start; i <= end; i++)
            {
                auxiliary[counter[CharAt(input[i], currentCharacter) + 1]++] = input[i];
            }
