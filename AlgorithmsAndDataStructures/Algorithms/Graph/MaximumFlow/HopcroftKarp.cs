...
#pragma warning restore CA1822 // Mark members as static
    {
        ...
        while (HasAugmentingPath(graph, leftSideVertices, pairs, distance))
            for (var i = 0; i < graph.Length; i++)
            {
                ...
                if (pairs[i] != -1 || !leftSideVertices.Contains(i)) continue;
                ...
                while (path.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!isEdgeExists) continue;

                        // Process edge currentVertex-position only if it is an edge in level graph.
                        if (!IsLevelGraphEdge(leftSideVertices, pairs, distance, currentVertex, w)) continue;
    ...
    private static bool IsLevelGraphEdge(ICollection<int> leftSetVertices, IReadOnlyList<int> pairs,
        IReadOnlyList<int> distance, int from, int to)
    {
        return distance[to] == distance[from] + 1 && IsResidualGraphEdge(leftSetVertices, pairs, from, to);
    }

    // Use BFS to find an augmenting path.
    private static bool HasAugmentingPath(IReadOnlyList<int[]> graph, ICollection<int> leftSideVertices,
        IReadOnlyList<int> pairs, IList<int> distance)
    {
        var visited = new bool[graph.Count];

        for (var i = 0; i < distance.Count; i++) distance[i] = int.MaxValue;
            ...
            for (var i = 0; i < graph.Count; i++)
            {
                ...
                if (IsResidualGraphEdge(leftSideVertices, pairs, currentVertex, i))
                    if (!visited[i])
                    {
                        ...
                        if (pairs[i] == -1) hasAugmentingPath = true;
                        ...
                        if (!hasAugmentingPath) queue.Enqueue(i);
                    }
            }
        }
    ...
    private static bool IsResidualGraphEdge(ICollection<int> leftSideVertices, IReadOnlyList<int> pairs, int from,
        int to)
    {
        // from isn't matched with to so forward edge isn't in the matching
        if (pairs[from] != to && leftSideVertices.Contains(from)) return true;
    ...
    {
        for (var i = 0; i < input.Count; i++) input[i] = -1;
    }
