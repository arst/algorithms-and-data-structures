            ...
            IList<int> lowestReachableDiscoveryTime)
        {
            ...
            foreach (var adjacentVertex in vertices[currentVertex])
            {
                if (!visited[adjacentVertex])
                {
                    ...
                    var result = IsBiConnected(vertices, adjacentVertex, currentDiscoveryTime, visited, parents, discoveryTimes, lowestReachableDiscoveryTime);
                    ...
                    lowestReachableDiscoveryTime[currentVertex] = Math.Min(lowestReachableDiscoveryTime[currentVertex], lowestReachableDiscoveryTime[adjacentVertex]);

                    if (parents[currentVertex] == NullParent && children > 1)
                    {
                        return false;
                    }
                    if (parents[currentVertex] != NullParent && lowestReachableDiscoveryTime[adjacentVertex] >= discoveryTimes[currentVertex])
                    {
                        return false;
                    }
                }
                ...
                {
                    lowestReachableDiscoveryTime[currentVertex] = Math.Min(lowestReachableDiscoveryTime[currentVertex], discoveryTimes[adjacentVertex]);
                }
            }
