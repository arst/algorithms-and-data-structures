...
public class BoruvkasAlgorithm
{
    public int GetMinimumSpanningTreeWeightParallel(int vertices, List<Edge> edges)
    {
        ...
        var numTrees = vertices;
        var mstWeight = 0;

        while (numTrees > 1)
        {
            Parallel.For(0, vertices, v => { cheapest[v] = null; });

            Parallel.ForEach(edges, edge =>
            {
                var set1 = Find(subsets, edge.Source);
                var set2 = Find(subsets, edge.Destination);
    ...
    public int GetMinimumSpanningTreeWeight(int vertices, List<Edge> edges)
    {
        ...
        var numTrees = vertices;
        var mstWeight = 0; // This will hold the total weight of the MST
