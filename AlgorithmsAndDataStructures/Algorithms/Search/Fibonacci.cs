...
public class Fibonacci<T> : ISearchAlgorithm<T> where T : IComparable<T>
{
    public int Search(T[] target, T value)
    {
        ...
        while (FibonacciNumber(startingFibonacciNumber) < target.Length) startingFibonacciNumber += 1;
        ...
        if (FibonacciNumber(startingFibonacciNumber) > 0 && target[offset + 1].CompareTo(value) == 0) return offset + 1;
