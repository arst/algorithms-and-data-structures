...
#pragma warning restore HAA0301 // Closure Allocation Source
    ...
    public void OpenAddressHashingMaintenanceHashTable()
    {
        ...
        for (var i = 0; i < 100000; i++) sut.Add(i.ToString(CultureInfo.InvariantCulture), i);
        for (var i = 0; i < 100000; i++) Assert.Equal(i, sut.Get(i.ToString(CultureInfo.InvariantCulture)));
        for (var i = 0; i < 100000; i++) sut.Add(i.ToString(CultureInfo.InvariantCulture), i * 2);
        for (var i = 0; i < 100000; i++) Assert.Equal(i * 2, sut.Get(i.ToString(CultureInfo.InvariantCulture)));
        for (var i = 0; i < 100000; i++) sut.Delete(i.ToString(CultureInfo.InvariantCulture));
        for (var i = 0; i < 100000; i++) Assert.False(sut.Find(i.ToString(CultureInfo.InvariantCulture)));
    }
}