...
#pragma warning disable HAA0302 // Display class allocation to capture closure
        ...
        for (var i = 0; i < 50; i++) threads[i] = new Thread(() => Ride("D"));

        for (var i = 50; i < 100; i++) threads[i] = new Thread(() => Ride("R"));

        for (var i = 0; i < 100; i++) threads[i].Start();

        for (var i = 0; i < 100; i++) threads[i].Join();
