...
public class SinglyLinkedListTests
{
    ...
    public void PrependCreatesReverseSimpleLinkedList()
    {
        ...
        for (var i = 5; i > -1; i--) sut.Prepend(i);

        var traversedSut = sut.Traverse();

        Assert.Equal(6, traversedSut.Count);

        for (var i = 0; i < traversedSut.Count; i++) Assert.Equal(i, traversedSut[i]);
    }
    ...
    public void TraverseRecursiveMatchesItterativeRaverse()
    {
        ...
        for (var i = 0; i < traversedSut.Count; i++) Assert.Equal(traversedSut[i], recursivelyTraversedSut[i]);
    }
