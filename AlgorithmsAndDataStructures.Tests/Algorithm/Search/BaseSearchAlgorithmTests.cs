...
public abstract class BaseSearchAlgorithmTests
{
    ...
    public void AlgorithmCantFindElementInZeroElementArray()
    {
        ...
        Assert.Equal(-1, sut.Search(Array.Empty<int>(), 1));
    }
    ...
    public void Fuzzy()
    {
        ...
        foreach (var item in toSearchNegative) Assert.Equal(-1, sut.Search(testData, item));

        foreach (var item in toSearchPositive) Assert.Equal(item.Item1, sut.Search(testData, item.Item2));
    }
