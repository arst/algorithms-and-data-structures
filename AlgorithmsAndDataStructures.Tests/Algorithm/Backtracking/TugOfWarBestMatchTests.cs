...
#pragma warning restore HAA0101 // Array allocation for params parameter
        ...
        public void BaseLineEven()
        {
            ...
            var (left, right) = sut.GetTug(set);
#pragma warning disable HAA0101 // Array allocation for params parameter
            Assert.Collection(left, arg => Assert.Equal(4, arg), arg => Assert.Equal(100, arg), arg => Assert.Equal(1, arg), arg => Assert.Equal(23, arg), arg => Assert.Equal(20, arg));
            Assert.Collection(right, arg => Assert.Equal(3, arg), arg => Assert.Equal(5, arg), arg => Assert.Equal(-3, arg), arg => Assert.Equal(89, arg), arg => Assert.Equal(54, arg));
#pragma warning restore HAA0101 // Array allocation for params parameter
        }
        ...
        public void BaseLineOdd()
        {
            ...
            var (left, right) = sut.GetTug(set);
#pragma warning disable HAA0101 // Array allocation for params parameter
            Assert.Collection(left, arg => Assert.Equal(23, arg), arg => Assert.Equal(-99, arg), arg => Assert.Equal(4, arg), arg => Assert.Equal(189, arg), arg => Assert.Equal(4, arg));
            Assert.Collection(right, arg => Assert.Equal(45, arg), arg => Assert.Equal(-34, arg), arg => Assert.Equal(12, arg), arg => Assert.Equal(0, arg), arg => Assert.Equal(98, arg), arg => Assert.Equal(-1, arg));
#pragma warning restore HAA0101 // Array allocation for params parameter
        }
    }
}
