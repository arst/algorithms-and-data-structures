...
public class TarjansAlgorithmTests
{
    ...
    public void EmptyGraphHashNoArticulationPoints()
    {
        ...
        Assert.Empty(sut.GetArticulationPoints(new UndirectedGraph(0)));
    }
    ...
    {
        var sut = new TarjansAlgorithm();
        Assert.Empty(sut.GetArticulationPoints(new UndirectedGraph(1)));
    }
    ...
    public void ThreeVerticeGraphHashOneArticulationPoint()
    {
        ...
        graph.Connect(0, 1);
        graph.Connect(1, 2);
        Assert.Collection(sut.GetArticulationPoints(graph), arg => Assert.Equal(1, arg));
    }
    ...
    public void NonDenseGraph()
    {
        ...
        graph.Connect(0, 3);
        graph.Connect(3, 4);
        Assert.Collection(sut.GetArticulationPoints(graph), arg => Assert.Equal(0, arg), arg => Assert.Equal(3, arg));
    }
    ...
    public void LinedUpGraph()
    {
        ...
        graph.Connect(1, 2);
        graph.Connect(2, 3);
        Assert.Collection(sut.GetArticulationPoints(graph), arg => Assert.Equal(1, arg), arg => Assert.Equal(2, arg));
    }
}