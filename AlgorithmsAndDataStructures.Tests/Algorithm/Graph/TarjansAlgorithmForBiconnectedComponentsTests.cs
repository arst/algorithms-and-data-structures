...
namespace AlgorithmsAndDataStructures.Tests.Algorithm.Graph
{
    public class TarjansAlgorithmForBiconnectedComponentsTests
    {
        ...
        public void EmptyGraphHashNoBiconnectedComponents()
        {
            ...
            var biconnectedComponents = sut.GetBiconnectedComponents(new Algorithms.Graph.Common.UndirectedGraph(0));
        ...
        {
            var sut = new TarjansAlgorithmForBiconnectedComponents();
            var biconnectedComponents = sut.GetBiconnectedComponents(new Algorithms.Graph.Common.UndirectedGraph(1));
            Assert.Collection(biconnectedComponents, arg => {
                Assert.Collection(arg, arg => Assert.Equal(0, arg));
            });
        }
